...
public class ItemMagicSword extends ItemSword implements IWorkbenchItem, ISpellCastingItem, IManaStoringItem {
    ...
    public boolean hitEntity(ItemStack stack, EntityLivingBase target, EntityLivingBase wielder) {
        ...
        //This is a magic sword. It doesn't lose durability as fast as a wand.
        if (level > 0 && mana > 0) this.consumeMana(stack, 1, wielder);
    ...
    public boolean shouldCauseReequipAnimation(ItemStack oldStack, ItemStack newStack, boolean slotChanged) {
        ...
        if (!oldStack.isEmpty() || !newStack.isEmpty()) {
            ...
            if (oldStack.getItem() == newStack.getItem() && !slotChanged && oldStack.getItem() instanceof ItemMagicSword
                    && newStack.getItem() instanceof ItemMagicSword
    ...
    public boolean canCast(ItemStack stack, Spell spell, EntityPlayer caster, EnumHand hand, int castingTick, SpellModifiers modifiers) {
        ...
        if (castingTick == 0) {
            if (MinecraftForge.EVENT_BUS.post(new SpellCastEvent.Pre(SpellCastEvent.Source.WAND, spell, caster, modifiers)))
                return false;
        } else {
            if (MinecraftForge.EVENT_BUS.post(new SpellCastEvent.Tick(SpellCastEvent.Source.WAND, spell, caster, modifiers, castingTick)))
                return false;
        }
    ...
    public boolean cast(ItemStack stack, Spell spell, EntityPlayer caster, EnumHand hand, int castingTick, SpellModifiers modifiers) {
        ...
        if (spell.cast(world, caster, hand, castingTick, modifiers)) {
            ...
            if (castingTick == 0)
                MinecraftForge.EVENT_BUS.post(new SpellCastEvent.Post(SpellCastEvent.Source.WAND, spell, caster, modifiers));
    ...
    public boolean itemInteractionForEntity(ItemStack stack, EntityPlayer player, EntityLivingBase entity, EnumHand hand) {
        ...
        if (player.isSneaking() && entity instanceof EntityPlayer && WizardData.get(player) != null) {
            String string = WizardData.get(player).toggleAlly((EntityPlayer) entity) ? "item." + ArcaneEssentials.MODID + ":sword.addally"
                    : "item." + ArcaneEssentials.MODID + ":sword.removeally";
