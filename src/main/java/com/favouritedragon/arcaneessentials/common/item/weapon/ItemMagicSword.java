...
public class ItemMagicSword extends ItemSword implements IWorkbenchItem, ISpellCastingItem, IManaStoringItem {
	...
	public ItemMagicSword(ToolMaterial material, Element element) {
		super(material);
	...
	public boolean hasEffect(ItemStack stack) {
		return !Wizardry.settings.legacyWandLevelling;
	}
...
//			text.add(status.getFormattingCode() + status.getDisplayName());
	...
	public SpellModifiers calculateModifiers(ItemStack stack, EntityPlayer player, Spell spell) {
		...
		if (this.element == spell.getElement()) {
			modifiers.set(SpellModifiers.POTENCY, 1.75F, true);
	...
	public boolean onApplyButtonPressed(EntityPlayer player, Slot centre, Slot crystals, Slot upgrade, Slot[] spellBooks) {
		...
		if (WandHelper.isWandUpgrade(upgrade.getStack().getItem())) {
			...
			if (WandHelper.getUpgradeLevel(centre.getStack(), specialUpgrade) < Constants.UPGRADE_STACK_LIMIT) {
