...
import static com.favouritedragon.arcaneessentials.common.util.DamageSources.*;
...
public class ArcaneEvents {
	...
	public static void onHurtEvent(LivingHurtEvent event) {
		if (event.getSource() instanceof IElementalDamage) {
			...
			if (((IElementalDamage) event.getSource()).getType() == MagicDamage.DamageType.RADIANT) {
				if (event.getEntity() instanceof EntityLivingBase && ((EntityLivingBase) event.getEntity()).isEntityUndead()) {
					event.setAmount(event.getAmount() * 1.5F);
