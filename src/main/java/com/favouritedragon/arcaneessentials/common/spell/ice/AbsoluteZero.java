...
public class AbsoluteZero extends ArcaneSpell {
    ...
    public boolean cast(World world, EntityPlayer caster, EnumHand hand, int ticksInUse, SpellModifiers modifiers) {
        ...
        List<EntityLivingBase> nearby = EntityUtils.getEntitiesWithinRadius(range, caster.posX, caster.getEntityBoundingBox().minY, caster.posZ, world, EntityLivingBase.class);
        nearby.remove(caster);
        if (!nearby.isEmpty()) {
            for (EntityLivingBase target : nearby) {
                if (AllyDesignationSystem.isValidTarget(caster, target)) {
                    ((BlockStatue) WizardryBlocks.ice_statue).convertToStatue((EntityLiving) target, caster, getProperty(EFFECT_DURATION).intValue() * 20);
                }
            }
        }
        if (world.isRemote) {
            for (int angle = 0; angle < 360; angle++) {
                ...
                ParticleBuilder.create(ParticleBuilder.Type.FLASH).pos(x + caster.posX, caster.posY + 0.5, z + caster.posZ).vel(x * range / 10,
                        world.rand.nextGaussian() / 10 * range, z * range / 10).time(14).clr(205, 254, 255).scale(particleSize).spawn(world);
            }
