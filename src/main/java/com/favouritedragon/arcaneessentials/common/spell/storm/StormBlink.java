        ...
        if (!world.isRemote) {
            if (result != null) {
                ...
                if (caster.attemptTeleport(pos.getX(), pos.getY() + 1, pos.getZ())) {
                    EntityUtils.playSoundAtPlayer(caster, SoundEvents.ENTITY_LIGHTNING_IMPACT, WizardrySounds.SPELLS,
                            2.0F + world.rand.nextFloat(), 1.0F + world.rand.nextFloat());
                    EntityUtils.playSoundAtPlayer(caster, WizardrySounds.ENTITY_LIGHTNING_SIGIL_TRIGGER, SoundCategory.AMBIENT, 4.0F, 1.0F);
                    List<EntityLivingBase> targets = EntityUtils
                            .getEntitiesWithinRadius(radius, caster.posX, caster.posY, caster.posZ, world, EntityLivingBase.class);
